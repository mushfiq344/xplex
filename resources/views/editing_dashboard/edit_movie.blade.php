    ...
    <div class='tabcontent' style="display:block;">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                @if($table=="movie")
                    <li class="breadcrumb-item"><a href="{{url('show_movies')}}">show Movies</a></li>
                @else
                    <li class="breadcrumb-item"><a href="{{url('show_requested_movies')}}">Movies For Approval</a></li>
                @endif

                <li class="breadcrumb-item active" aria-current="page">Edit Movie</li>
            </ol>
        </nav>
        <form action="{{url('save_edited_movie')}}" method="POST">
            @csrf
            <input type="text" name="id" value="{{$id}}" required autofocus style="display: none;">
            <input type="text" name="table_name" value="{{$table}}" required autofocus style="display: none;">
            <input type="text" name="type" value="{{$type}}" required autofocus style="display: none;">
            <h4> Title: </h4>
            <input type="text" id="title" class="form-control" name="title" value="<?php echo $title  ?>" required
                   autofocus><br>
            <h4> imdb Title: </h4>
            <input type="text" id="imdb_title" class="form-control" name="imdb_title" value="{{$imdb_title}}" required
                   autofocus><br>
            <div class="row ml-0" style="padding: 0;margin: 0">
                <div class="cold-md-3 ml-4">
                    <h4> Release Date: </h4>
                    <input type="date" id="released" class="form-control" name="released" value="{{$released}}" required
                           autofocus>
                </div>
                <div class="cold-md-3 ml-4">
                    <h4> Year: </h4>
                ...
                <div class="col-md-3 ml-3 mr-0">
                    ...
                    <input type="text" id="director"
                           class="form-control{{ $errors->has('director') ? ' is-invalid' : '' }}" name="director"
                           value="<?php echo $director  ?>" required autofocus>
                </div>
            </div>
            <br>
            <h4>Actors : </h4>
            <input type="text" id="actors" class="form-control{{ $errors->has('Production') ? ' is-invalid' : '' }}"
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    <input type="text" id="production"
                           class="form-control{{ $errors->has('production') ? ' is-invalid' : '' }}" name="production"
                           value="<?php echo $production  ?>" required autofocus>
                </div>
                <div class="col-md-4">
                    <h4>Language : </h4>
                    <input type="text" id="language"
                           class="form-control{{ $errors->has('language') ? ' is-invalid' : '' }}" name="language"
                           value="<?php echo $language ?>" required autofocus>
                </div>
                <div class="col-md-4">
                    <h4>Genre : </h4>
                    <input type="text" id="genre" class="form-control{{ $errors->has('genre') ? ' is-invalid' : '' }}"
                           name="genre" value="<?php echo $genre  ?>" required autofocus>
                </div>
            </div>
            <br>
            <h4>Plot : </h4>
            <input type="text" id="plot" class="form-control{{ $errors->has('plot') ? ' is-invalid' : '' }}" name="plot"
            ...
            <img id="poster_frame" value="" name="poster_frame" src="{{url($poster_url_value)}}" width="500"
                 height="750">
            <!-- Cover -->
            <div class="input-group mb-2" style="width: 85%">
            ...
            <input type="text" name="download_link_movie" value="{{$download_link}}">
