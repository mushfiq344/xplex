...
<head>
    ...
    <!-- popup -->
    <link rel="stylesheet" type="text/css"
          href="{{asset('../assets/bower_components/magnific-popup/dist/magnific-popup.css')}}"/>
...
<section class="content-wrap">
    ...
    <div class="container-fluid youplay-store store-grid" id="galleryCategory" category="{{$category}}"
         type="{{$type}}">

        <h4>{{strtoupper($item)}} > {{strtoupper($type)}} > {{strtoupper($category)}} </h4>
        ...
        <div class="col-md-9">
            ...
            <!-- Single Product Block -->
                <div class="item col-lg-2 col-md-4 col-xs-6 active" data-filters="sob">
                    <a id="id" href="{{url('movie',[$value->id])}}" class="angled-img">
        ...
        <div class="col-md-3">
            ...
            <div class="side-block">
                ...
                <ul class="block-content">
                    ...
                    <div class="col-md-4">
                        <a href="{{url('/galleryGenreMovie',['movie','all',$genre[$i]->genre_type])}}">{{$genre[$i]->genre_type}}</a>
                    </div>

                    <div class="col-md-4">
                        <a href="{{url('/galleryGenreMovie',['movie','all',$genre[$i+1]->genre_type])}}">{{$genre[$i+1]->genre_type}}</a>
                    </div>

                    <div class="col-md-4">
                        <a href="{{url('/galleryGenreMovie',['movie','all',$genre[$i+2]->genre_type])}}">{{$genre[$i+2]->genre_type}}</a>
                    </div>
                    <br>
                    <?php $i = $i+3; } ?>
                </ul>
...
<!-- popup -->
<script type="text/javascript"
        src="{{asset('../assets/bower_components/magnific-popup/dist/jquery.magnific-popup.min.js')}}"></script>
...
<script>
    $(document).ready(function () {
        $('#new_data').click(function () {
            window.location = "{{URL::to('/gallery',['movie',$type,'recent'])}}";
        });
        $('#most_popular_data').click(function () {
            window.location = "{{URL::to('/gallery',['movie',$type,'popular'])}}";
        });
        $('#imdb_rating_data').click(function () {
            window.location = "{{URL::to('/gallery',['movie',$type,'imdb'])}}";
        });
        $('#rotten_rating_data').click(function () {
            window.location = "{{URL::to('/gallery',['movie',$type,'rottentomatoes'])}}";
        });
        $('#all_data').click(function () {
            window.location = "{{URL::to('/gallery',['movie',$type,'all'])}}";
        });
    });
</script>
